...
void Collisions::update() {
	...
	if (currSpeed.right > 0 && (activeSonars & SONAR_FRONT_RIGHT)) {
		if (activeCounts[0] < TRESHOLD_COUNT) activeCounts[0]++;
	}
	else {
		if (activeCounts[0]) activeCounts[0]--;
	}

	if (currSpeed.right < 0 && (activeSonars & SONAR_BACK_RIGHT)) {
		if (activeCounts[1] < TRESHOLD_COUNT) activeCounts[1]++;
	}
	else {
		if (activeCounts[1]) activeCounts[1]--;
	}

	if (currSpeed.left > 0 && (activeSonars & SONAR_FRONT_LEFT)) {
		if (activeCounts[2] < TRESHOLD_COUNT) activeCounts[2]++;
	}
	else {
		if (activeCounts[2]) activeCounts[2]--;
	}

	if (currSpeed.left > 0 && (activeSonars & SONAR_BACK_LEFT)) {
		if (activeCounts[3] < TRESHOLD_COUNT) activeCounts[3]++;
	}
	else {
